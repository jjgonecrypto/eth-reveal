...
const tryAddABIAndMaybeProxyTarget = async ({ contract, etherscanKey }) => {
	const { abi, contractName, error } = await tryGetABIAndSource({ contract, etherscanKey });
	...
	if (contractInstance.target) {
		...
		const { abi, contractName: targetContractName } = await tryGetABIAndSource({
			contract: target,
			etherscanKey,
		});
		if (abi) {
...
const lookup = async ({ hash, etherscanKey = process.env.ETHERSCAN_API_KEY }) => {
	...
	const { to, data, value, nonce, gasPrice, gasLimit } = tx;
